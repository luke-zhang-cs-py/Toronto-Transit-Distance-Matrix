...
has already shipped two such claims: the README's project layout said 112
tests and 89% coverage while the suite had 167 and 93%. So the figures are
...
What is checked statically, so it works in CI with no coverage run:
  ...
  * its statement count is what `coverage` counts, using this repo's own
    `.coveragerc` — which is what makes the two omitted build tools stay
    omitted rather than quietly appearing;
